...
public class ProjectCreateServiceImpl implements ProjectCreateService {
    ...
    public void saveProjectInfo(ProjectStatusEnume auth, ProjectRedisStorageVo project) {
        ...
        List<String> detailImage =project.getDetailsImage();
        //2.2)、保存详情图
        for (String string :detailImage){
            TProjectImages tProjectImages1 = new TProjectImages(null, projectId, headerImage, ProjectImageTypeEnume.DETAILS.getCode());
            projectImagesMapper.insertSelective(tProjectImages1);
        }
        //3、保存项目的标签信息
        List<Integer> tagids = project.getTagids();
        for (Integer tid :tagids){
            TProjectTag tProjectTag = new TProjectTag(null, projectId, tid);
